...
SCHOOL_NAME, TAG = "中南大学", "CSU"
BASE_URL = "https://career.csu.edu.cn"
LIST_URL = f"{BASE_URL}/campus/index"
MAX_PAGES = 200
SUPPORTED_FILTERS = {"company": "local", "position": "native", "keyword": "native", **{k: "unsupported" for k in ("location", "company_nature", "industry", "education", "position_type", "employment_type", "student_type")}}
...
def _unpack(text: str) -> str:
    ...
    if not match:
        return ""
    stage1 = zlib.decompress(base64.b64decode(match.group(1))).decode("utf-8")[int(match.group(2)):]
    return base64.b64decode(stage1).decode("utf-8")[int(match.group(3)):]


def _records(content: bytes) -> list[dict[str, str]]:
    soup = BeautifulSoup(content, "lxml")
    result = []
    for row in soup.select(".campus-index-front-list ul.infoList"):
        link = row.select_one('a[href*="/campus/view/id/"]')
        cells = row.select("li")
        if link and cells:
            result.append({"title": link.get_text(" ", strip=True), "date": cells[-1].get_text(" ", strip=True), "url": urljoin(BASE_URL, link.get("href", "")), "top": "置顶" in row.get_text()})
    return result
...
def _detail(content: bytes) -> tuple[str, str]:
    ...
    packed = _unpack(text)
    body = packed or str(soup.select_one(".aContent") or "")
...
def collect(start: datetime, end: datetime, filters: dict[str, str], raw_dir: Path) -> list[dict[str, str]]:
    ...
    if title and keyword and title != keyword:
        raise ValueError(f"{SCHOOL_NAME} position 与 keyword 共用站内 keyword 参数")
    raw_dir.mkdir(parents=True, exist_ok=True)
    pages = older_pages = detail_success = detail_failed = 0
    candidates: list[dict[str, str]] = []
    with HttpClient() as client:
        for page in range(1, MAX_PAGES + 1):
            response = client.get(LIST_URL, params={"category": "1", "city": "", "keyword": title or keyword, "page": page})
            ...
            if not records:
                break
            normal_dates = [parse_source_datetime(r["date"]) for r in records if not r["top"]]
            candidates.extend(r for r in records if in_range(r["date"], start, end))
            older_pages = older_pages + 1 if normal_dates and all(d < start for d in normal_dates) else 0
            if older_pages >= 2:
                break
        results = []
    ...
    LAST_STATS = {"strategy": "get_html_packed_detail", "pages": pages, "matched": len(results), "details_success": detail_success, "details_failed": detail_failed}
    return results
